...
(() => {
  ...
  const email = document.getElementById('email'), password = document.getElementById('password'), tree = document.getElementById('tree');
  let user = null, activeList = null, activeApp = 'lists', allLists = [];
  ...
  oldItems.replaceWith(gridContainer);
  const grid = new Grid({
    ...
    renderCell: (value, row, column) => {
      ...
      if (column.key === 'completed') return value ? '✓' : '';
      return '';
    }
  });
  ...
  function renderAppTree(name) {
    ...
    apps[name].nodes.forEach(node => { const row = document.createElement('div'); row.className = 'eb-tree-row eb-tree-child'; const button = document.createElement('button'); button.type='button'; button.className='eb-tree-node'; button.textContent='• ' + node.name; button.onclick=()=>openExternal(node.url,node.name); row.append(button); tree.append(row); });
  }

  function openExternal(url,title) { document.getElementById('listWorkspace').hidden=true; document.getElementById('appTitle').textContent=title; const frame=document.getElementById('contentFrame'); frame.src=url; frame.hidden=false; }
  ...
  function treeButton(list, depth, context = {}) {
    ...
    if (context.hasChildren) { const toggle=document.createElement('button'); toggle.type='button'; toggle.className='eb-tree-action'; toggle.textContent=context.expanded?'▾':'▸'; toggle.title=context.expanded?'Collapse':'Expand'; toggle.setAttribute('aria-label',toggle.title); toggle.onclick=e=>{e.stopPropagation();context.toggle();}; wrap.append(toggle); }
    else { const spacer=document.createElement('span'); spacer.className='eb-tree-action'; spacer.textContent=' '; spacer.setAttribute('aria-hidden','true'); wrap.append(spacer); }
    const b=document.createElement('button'); b.type='button'; b.className='eb-tree-node'; b.textContent=(depth?'• ':'')+`${list.name} ${list.ordered?'☷':'☰'}`; b.title=list.ordered?'Ordered list — click to open':'Unordered list — click to open'; b.setAttribute('aria-current',String(activeList?.id===list.id)); b.onclick=()=>openList(list); wrap.append(b);
    const actions=document.createElement('div'); actions.className='eb-tree-actions'; actions.append(actionButton('+','Add sub-list',()=>addSubList(list)), actionButton('↑','Move up',()=>moveList(list,-1)), actionButton('↓','Move down',()=>moveList(list,1)), actionButton('×','Delete list',()=>deleteList(list))); wrap.append(actions); row.append(wrap); return row;
  }

  async function renderListTree() {
    document.getElementById('contentFrame').hidden=true; document.getElementById('listWorkspace').hidden=false; document.getElementById('appTitle').textContent='Lists';
    const {data,error}=await supabase.from('lists').select('*').order('position').order('created_at'); if(error)return setStatus(error.message); allLists=data||[];
    if (!treeInitialized) { allLists.forEach(list => { if (allLists.some(child => child.parent_list_id === list.id)) treeView.expandedIds.add(list.id); }); treeInitialized = true; }
    treeView.setData(allLists); window.dispatchEvent(new Event('eb:lists-rendered'));
  }
  ...
  async function persistListOrder(orderedLists){for(let index=0;index<orderedLists.length;index++){const list=orderedLists[index];const r=await supabase.from('lists').update({position:index}).eq('id',list.id).eq('owner_id',user.id);if(r.error){setStatus(r.error.message);return false;}}return true;}
  ...
  async function toggleItem(item){const r=await supabase.from('list_items').update({completed:!item.completed}).eq('id',item.id).eq('owner_id',user.id);if(r.error)return setStatus(r.error.message);await refreshItems();}
  ...
  async function renameItem(item){const text=prompt('Edit item text:',item.text);if(text===null)return;if(!text.trim())return;const r=await supabase.from('list_items').update({text:text.trim()}).eq('id',item.id).eq('owner_id',user.id);if(r.error)return setStatus(r.error.message);await refreshItems();}
  ...
  async function refreshItems(){if(!activeList)return;const{data,error}=await supabase.from('list_items').select('*').eq('list_id',activeList.id).order('position').order('created_at');if(error)return setStatus(error.message);const listData=data||[];grid.setRows(listData.map(item=>({...item,completed:!!item.completed,actions:itemActions(item)})));}
  ...
  document.getElementById('listsNav').onclick=()=>selectApp('lists');document.getElementById('infoNav').onclick=()=>selectApp('info');document.getElementById('supportableNav').onclick=()=>selectApp('support');document.getElementById('listOrderToggle').onclick=toggleListOrdered;
