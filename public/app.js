...
(() => {
  ...
  // Reusable Tree boundary. Lists remains the data owner; Tree owns tree behavior.
  ...
  function treeButton(list, depth, context = {}) {
    ...
    const b=document.createElement('button'); b.type='button'; b.className='eb-tree-node'; b.textContent=(depth?'• ':'')+`${list.name} ${list.ordered?'☷':'☰'}`; b.title=list.ordered?'Ordered list — click to open':'Unordered list — click to open'; b.setAttribute('aria-current',String(activeList?.id===list.id)); b.onclick=()=>{context.select?.();openList(list);};
    wrap.append(b);
  ...
  async function moveList(list,direction){const siblings=allLists.filter(l=>(l.parent_list_id||null)===(list.parent_list_id||null)).sort((a,b)=>a.position-b.position); const index=siblings.findIndex(l=>l.id===list.id), target=index+direction; if(index<0||target<0||target>=siblings.length)return; const other=siblings[target]; let r=await supabase.from('lists').update({position:other.position}).eq('id',list.id).eq('owner_id',user.id); if(r.error)return setStatus(r.error.message); r=await supabase.from('lists').update({position:list.position}).eq('id',other.id).eq('owner_id',user.id); if(r.error)return setStatus(r.error.message); await renderListTree();}
