...
{
    console.log("SIGNEDIN: received user =", user);
    console.log("SIGNEDIN: state.user BEFORE =", state.user);
    console.log("SIGNEDIN: authout.hidden BEFORE =", $('authout').hidden);
    console.log("SIGNEDIN: authin.hidden BEFORE =", $('authin').hidden);
    console.log("SIGNEDIN: app.hidden BEFORE =", $('app').hidden);
    state.user = user;
    $('authout').hidden = !user;
    $('authin').hidden = !!user;
    $('app').hidden = !user;
    $('user').textContent = user?.email || '';
    console.log("SIGNEDIN: state.user AFTER =", state.user);
    console.log("SIGNEDIN: authout.hidden AFTER =", $('authout').hidden);
    console.log("SIGNEDIN: authin.hidden AFTER =", $('authin').hidden);
    console.log("SIGNEDIN: app.hidden AFTER =", $('app').hidden);
...
async function signIn() 
{
    try 
    {
        console.log("SIGNIN: starting", $('email').value.trim());
        console.log("SIGNIN: state.user BEFORE =", state.user);
        status('Signing in…');
        ...
        console.log("SIGNIN: auth response =", d);
        console.log("SIGNIN: user from response =", d?.user);
        console.log("SIGNIN: session from response =", d?.session);
        state.session = d;
        localStorage.setItem('eb_session', JSON.stringify(d));
        console.log("SIGNIN: before signedIn(), state.user =", state.user);
        signedIn(d.user);
        console.log("SIGNIN: after signedIn(), state.user =", state.user);
        await loadLists();
        status('')
    }
    catch (e) 
