(() => {
  ...
  let user = null, activeList = null, activeApp = 'lists';
  const setStatus = text => status.textContent = text || '';

  const apps = {
    lists: { name: 'Lists', icon: '☷', nodes: [] },
    info: { name: 'Info', icon: 'ⓘ', nodes: [{ id: 'about', name: 'About', url: 'https://ebliss-info.emptinessbliss15.workers.dev/' }] },
    support: { name: 'Supportable', icon: '◉', nodes: [{ id: 'supportable', name: 'Supportable', url: 'https://supportable.emptinessbliss15.workers.dev/' }] }
  };

  function selectApp(name) {
    activeApp = name;
    document.getElementById('listsNav').setAttribute('aria-current', name === 'lists' ? 'true' : 'false');
    document.getElementById('infoNav').setAttribute('aria-current', name === 'info' ? 'true' : 'false');
    document.getElementById('supportableNav').setAttribute('aria-current', name === 'support' ? 'true' : 'false');
  ...
  function renderAppTree(name) {
    ...
    apps[name].nodes.forEach(node => {
      ...
      button.type = 'button'; button.className = 'eb-tree-node'; button.textContent = node.name;
      button.onclick = () => openExternal(node.url, node.name);
  ...
  function openExternal(url, title) {
    ...
    document.getElementById('contentFrame').hidden = false;
  }
  ...
  async function renameActiveList() {
    ...
    activeList.name=name; showActiveListName(); await renderListTree();
  }
