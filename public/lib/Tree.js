...
export class Tree {
  constructor({ container, getId = node => node.id, getParentId = node => node.parentId ?? null, renderNode }) {
    ...
    this.container.addEventListener('pointerdown',e=>{const h=e.target.closest?.('.eb-tree-drag-handle');if(h){const r=h.closest?.('.eb-tree-row[data-tree-node-id]');if(r)this.dragArmed=true;}});
