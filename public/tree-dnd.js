(() => {
  ...
  let busy=false,enhanceTimer=null;
  const collapsed=new Set(JSON.parse(localStorage.getItem('eb-tree-collapsed')||'[]'));
  function saveCollapsed(){localStorage.setItem('eb-tree-collapsed',JSON.stringify([...collapsed]));}
  function applyCollapsed(){const rows=[...document.querySelectorAll('#tree .eb-tree-entry[data-list-id]')];const byId=new Map(rows.map(r=>[r.dataset.listId,r]));rows.forEach(r=>{r.parentElement.style.display='';let p=r.dataset.parentId;while(p){if(collapsed.has(p)){r.parentElement.style.display='none';break}p=byId.get(p)?.dataset.parentId||null;}});rows.forEach(r=>{const b=r.querySelector('.eb-tree-collapse');if(b)b.textContent=collapsed.has(r.dataset.listId)?'▸':'▾';});}
  async function hydrateIds(){const tree=document.getElementById('tree');if(!tree)return false;const {data,error}=await db.from('lists').select('id,name,parent_list_id,position').order('position').order('created_at');if(error)return false;const byName=new Map(),children=new Set();(data||[]).forEach(x=>{if(!byName.has(x.name))byName.set(x.name,[]);byName.get(x.name).push(x);if(x.parent_list_id)children.add(x.parent_list_id);});tree.querySelectorAll('.eb-tree-entry').forEach(entry=>{const node=entry.querySelector('.eb-tree-node');if(!node)return;const name=node.textContent.replace(/^•\s*/,'').replace(/\s+·\s+(ordered|unordered)$/,'').replace(/\s+[☷☰]$/,'');const candidates=byName.get(name)||[];if(candidates.length===1){entry.dataset.listId=candidates[0].id;entry.dataset.parentId=candidates[0].parent_list_id||'';entry.dataset.hasChildren=children.has(candidates[0].id)?'1':'0';}});return true;}
  async function persistGroup(rows,parentId){for(let i=0;i<rows.length;i++){const r=await db.from('lists').update({parent_list_id:parentId||null,position:i});if(r.error)return r.error}return null;}
  function clearTargets(){document.querySelectorAll('#tree .eb-tree-drop-before,#tree .eb-tree-drop-after,#tree .eb-tree-drop-child').forEach(x=>x.classList.remove('eb-tree-drop-before','eb-tree-drop-after','eb-tree-drop-child'));}
  async function refreshTree(){const nav=document.getElementById('listsNav');if(nav)nav.click();}
  async function enhance(){const tree=document.getElementById('tree');if(!tree||busy)return;if(!(await hydrateIds()))return;tree.querySelectorAll('.eb-tree-row').forEach(row=>{const entry=row.querySelector('.eb-tree-entry'),node=row.querySelector('.eb-tree-node');if(!entry||!node)return;const id=entry.dataset.listId;if(!id||entry.dataset.dragEnhanced==='1')return;entry.dataset.dragEnhanced='1';
      let collapse=entry.querySelector('.eb-tree-collapse');
      if(entry.dataset.hasChildren==='1'&&!collapse){collapse=document.createElement('button');collapse.type='button';collapse.className='eb-tree-collapse';collapse.textContent=collapsed.has(id)?'▸':'▾';collapse.title='Expand/collapse';collapse.setAttribute('aria-label','Expand/collapse');collapse.onclick=e=>{e.stopPropagation();if(collapsed.has(id))collapsed.delete(id);else collapsed.add(id);saveCollapsed();applyCollapsed();};entry.insertBefore(collapse,node);}
      let handle=entry.querySelector('.eb-tree-drag-handle');
      if(!handle){handle=document.createElement('button');handle.type='button';handle.className='eb-tree-drag-handle';handle.textContent='⠿';handle.title='Drag to reorder or reparent';handle.setAttribute('aria-label',handle.title);entry.insertBefore(handle,entry.firstChild);}
      handle.addEventListener('mousedown',()=>{entry.draggable=true;});
      entry.addEventListener('dragstart',e=>{if(e.target!==handle){e.preventDefault();return;}e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',id);entry.classList.add('eb-tree-dragging');});
      entry.addEventListener('dragend',()=>{entry.classList.remove('eb-tree-dragging');clearTargets();entry.draggable=false;});
      ...
      entry.addEventListener('drop',async e=>{e.preventDefault();e.stopPropagation();const dragging=tree.querySelector('.eb-tree-dragging');if(!dragging||dragging===entry)return;const dragId=dragging.dataset.listId,targetId=entry.dataset.listId,r=entry.getBoundingClientRect(),y=e.clientY-r.top;clearTargets();busy=true;const {data,error}=await db.from('lists').select('id,name,parent_list_id,position').order('position').order('created_at');if(error){busy=false;return}const all=data||[],target=all.find(x=>x.id===targetId),dragged=all.find(x=>x.id===dragId);if(!target||!dragged){busy=false;return}let p=target.parent_list_id,cycle=targetId===dragId;while(p&&!cycle){if(p===dragId)cycle=true;p=all.find(x=>x.id===p)?.parent_list_id||null}if(cycle){busy=false;return}const childDrop=y>=r.height*.25&&y<=r.height*.75,newParent=childDrop?targetId:(target.parent_list_id||null);const siblings=all.filter(x=>(x.parent_list_id||null)===(newParent||null)&&x.id!==dragId).sort((a,b)=>a.position-b.position);let insert=siblings.length;if(!childDrop){const ti=siblings.findIndex(x=>x.id===targetId);insert=y<r.height*.5?Math.max(0,ti):ti+1;}siblings.splice(insert,0,dragged);const saveErr=await persistGroup(siblings,newParent);busy=false;if(saveErr)return;await refreshTree();});
    });applyCollapsed();}
